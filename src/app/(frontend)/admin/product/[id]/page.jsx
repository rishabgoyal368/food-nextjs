  ...
  productImages: z
    .array(
      z.object({
        file: z
          .instanceof(File)
          .refine((file) => file.size <= 2 * 1024 * 1024, "Each file must be less than 2MB"), // Limit file size
      })
    )
    .optional(), // Make optional; validation happens later
});
...
function AddProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      const response = await axios.get(`/api/admin/product?name=${id}`);
      if (response.data && response.data.data) {
        const userData = response.data.data[0];
        form.setValue("productName", userData.productName || "");
        form.setValue("categoryId", userData.categoryId || "");
        form.setValue("price", userData.price || "");
        form.setValue("discount", userData.discount || "");
        form.setValue("description", userData.description || "");

        const imageUrls =
          userData?.productImages?.map((image) => `${image}`) || [];
